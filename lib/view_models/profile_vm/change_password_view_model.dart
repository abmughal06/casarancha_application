import 'package:casarancha/view_models/common_view_model.dart';
import 'package:flutter/material.dart';

import '../../resources/color_resources.dart';
import '../../resources/image_resources.dart';

class ChangePasswordViewModel extends CommonViewModel {
  FocusNode currentPasswordFocus = FocusNode();
  FocusNode newPasswordFocus = FocusNode();
  FocusNode confirmPasswordFocus = FocusNode();
  String? icCurrentPwdSvg, icNewPasswordSvg, icConfirmPwdSvg;
  Color? currentPwdFillClr, newPasswordFillClr, confirmPwdFillClr;
  Color? currentPwdBorderClr, newPasswordBorderClr, confirmPwdBorderClr;

  bool currentPasswordVisible = true , newPasswordVisible = true , confirmPasswordVisible = true;


  setCurrentPwdVisibility() {
    currentPasswordVisible = !currentPasswordVisible;
    icCurrentPwdSvg == icShowPassword
        ? icCurrentPwdSvg = icHidePassword
        : icCurrentPwdSvg = icShowPassword;
    notifyListeners();
  }
  setNewPwdVisibility() {
    newPasswordVisible = !newPasswordVisible;
    icNewPasswordSvg == icShowPassword
        ? icNewPasswordSvg = icHidePassword
        : icNewPasswordSvg = icShowPassword;
    notifyListeners();
  }
  setConfirmPwdVisibility() {
    confirmPasswordVisible = !confirmPasswordVisible;
    icConfirmPwdSvg == icShowPassword
        ? icConfirmPwdSvg = icHidePassword
        : icConfirmPwdSvg = icShowPassword;
    notifyListeners();
  }

  setCurrentPwdFocusChange() {
    if (currentPasswordFocus.hasFocus) {
      currentPwdFillClr = colorFDF;
      currentPwdBorderClr = colorF73;
    } else {
      currentPwdFillClr = colorFF3;
      currentPwdBorderClr = null;
    }

    notifyListeners();
  }

  setNewPwdFocusChange() {
    if (newPasswordFocus.hasFocus) {
      newPasswordFillClr = colorFDF;
      newPasswordBorderClr = colorF73;
    } else {
      newPasswordFillClr = colorFF3;
      newPasswordBorderClr = null;
    }

    notifyListeners();
  }

  setConfirmPwdFocusChange() {
    if (confirmPasswordFocus.hasFocus) {
      // icPasswordSvg = icSelectLock;
      confirmPwdFillClr = colorFDF;
      confirmPwdBorderClr = colorF73;
    } else {
      // icPasswordSvg = icDeselectLock;
      confirmPwdFillClr = colorFF3;
      confirmPwdBorderClr = null;
    }

    notifyListeners();
  }
}
